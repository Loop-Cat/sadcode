...
class PathingManager {
		...
		if (!fatigue) {
			...
			if (newPath) {
				...
				if (
					...
					matrixCallbacks.push(matrix => this.addWorkingCreepsToMatrix(matrix, room, priority));
				}

				if (avoidLocalHostiles) {
					matrixCallbacks.push(matrix => this.addHostilesToMatrix(matrix, creepPos, room, 2));
				}

				const avoid = options.avoid;
				if (avoid) {
					matrixCallbacks.push(matrix => this.addAvoidPositionsToMatrix(matrix, creepRoomName, avoid));
				}

				if (matrixCallbacks.length > 0) {
					...
					if (prevCostCallback) {
						costCallback = (roomName, _matrix) => {
							...
							if (roomName === creepRoomName) {
								...
								for (const matrixFn of matrixCallbacks) {
									matrixFn(matrix || _matrix);
								}
							}
							if (matrix) {
								return matrix;
	...
	isCreepCanMove(creep) {
		if (creep.hits === creep.hitsMax || !creep.body) {
			return true;
		}
	...
	* getAdjacentPositions(pos) {
	...
	getCreepMovePos(creep, priority, moves, pathEnd) {
		...
		for (const pos of this.getAdjacentPositions(creepPos)) {
			const {x, y} = pos;
			let cost = matrix.get(x, y) || terrain.getCost(x, y);
			if (cost === 255 || this.hasMove(pos, moves, priority)) {
				continue;
			}
			if (this.hasObstacleCreep(room, x, y)) {
				cost = 1000;
			} else if (pathEnd) {
	...
	addAvoidPositionsToMatrix(matrix, roomName, avoid) {
		for (const target of avoid) {
			const pos = target.pos || target;
			if (pos.roomName && pos.roomName !== roomName) {
				continue;
			}
			const {x, y, cost = 255} = pos;
			matrix.setFast(x, y, cost);
		}
	}
	...
	getTargetRangePositions(targetPos, range, options) {
		...
		const matrix = roomCallback(roomName);
		const positions = [];
		for (let x = minX; x <= maxX; x++) {
			positions.push({x: x, y: minY, roomName});
			positions.push({x: x, y: maxY, roomName});
		}
		for (let y = minY + 1; y <= maxY - 1; y++) {
			positions.push({x: minX, y: y, roomName});
			positions.push({x: maxX, y: y, roomName});
		}
		const targets = [];
		for (const pos of positions) {
			const {x, y} = pos;
			const cost = (matrix && matrix.get(x, y)) || terrain.getCost(x, y);
			if (cost < 255) {
	...
	fixPath(path, matrix, roomName) {
		...
		for (let i = 0; i < path.length; i++) {
			...
			const {x: posX, y: posY} = pos;
			let cost = matrix.get(posX, posY) || terrain.getCost(posX, posY);
			if (
				i >= 2 && cost === 1 &&
				costs[i - 1] !== 1 &&
				costs[i - 2] === 1
...
if (!Creep.prototype.originalMoveTo) {
	...
	Creep.prototype.moveTo = function(target, defaultOptions = {}) {
		...
		const avoid = [Game.flags['Spot_1'], Game.flags['Spot_2'], Game.flags['Spot_3'], Game.flags['Spot_4'], Game.flags['Spot_5'], Game.flags['Spot_6']];
 ...
 * @returns {Number}
 */
/* Creep.prototype.moveToRange = function(target, range, options) {
	if (this.pos.inRangeTo(target, range)) {
		return IN_RANGE;
	}
	return this.moveTo(target, { range, ...options });
}
...
Creep.prototype.moveToRoom = function(roomName, target = undefined, options = {}) {
	if (this.room.name === roomName && !Utils.isPosExit(this.pos)) {
		return IN_ROOM;
	}
	if (!target) {
